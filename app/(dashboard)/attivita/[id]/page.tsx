...
export default async function AttivitaDetailPage({
  ...
  const [attivita, sessioni, iscrizioni, bambini] = await Promise.all([
    getAttivita(id),
        ...
        <CardContent>
          ...
          >
            <Button
              variant="destructive"
              type="submit"
              disabled={iscrizioni.length > 0 || sessioni.length > 0}
            >
              Elimina attività
            </Button>
            {(iscrizioni.length > 0 || sessioni.length > 0) && (
              <p className="mt-2 text-xs text-[var(--muted-foreground)]">
                Rimuovi prima sessioni e iscrizioni collegate.
              </p>
            )}
          </form>
        </CardContent>
