...
>
> Ultimo aggiornamento: **2026-05-15** — **Rimossa la rotta `/dashboard`**: la home dell'area educativa diventa `/attivita` (prima voce della sidebar edu, riordinata). `homeForRuolo(ruolo)` in `lib/config.ts` decide la destinazione del redirect post-login: admin/volontario_cassa → `/cassa`, coordinatore_educativo → `/attivita`. Il flusso passa per `app/page.tsx` (la rotta `/`) che fa il dispatch. Aggiornati: proxy `auth.config.ts` (redirect post-login, post-primo-accesso, fuori scope coordinatore), `page-guards.ts` (`requireAdminOrCoordinatore`/`requireAdminOrCassa` redirigono a `/attivita`), `lib/actions/utenti.ts` (`primoAccessoAction` → `/`), `lib/actions/auth.ts` (`signIn redirectTo="/"`), `lib/actions/disponibilita.ts` (rimosso `revalidatePath("/dashboard")`). 31 rotte (-1).  **Sessione precedente della giornata**: separato saldi reali conto da rendiconto finanziario, nuova rotta `/conti` con i saldi REALI per Cassa/BCC/SumUp **inclusi i giroconti**, `/cassa` diventa la vista rendiconto finanziario con 3 KPI + chart mensile via `recharts`. Caso reale: BCC oggi mostrava −5560,71€, saldo reale −2445,71€. Branch: `claude/fix-sumup-statement-import-5coBD` (continua dalla sessione import SumUp+BCC + saldi/rendiconto). Aggiornamento precedente del 2026-05-16: pricing iscrizioni rifondato (modalita `tipo_prezzo`, quota una-tantum, sconti). **SECURITY_PLAN**: in attesa la sola sessione 8 (DB transactions, rischio 🔴 alto).
>
> **PR mergeate oggi 2026-05-15** (in ordine cronologico):
> - PR #38 + #39: fix accesso utente (loop /primo-accesso + signOut buggato) — non SECURITY_PLAN, hot-fix da segnalazione utente.
> - PR #41 (`6375505`): logo /login + favicon col logo ACLI (proxy matcher allargato, `app/icon.png` sostituisce `app/favicon.ico`).
> - PR #42 (`fbeaa5f`): **CSP flip Report-Only → enforcing** (chiude la Sessione 1 SECURITY_PLAN dopo finestra 24h+).
> - PR #43 (`23e327b`): **Sessione 5** — JWT maxAge 14gg→24h + session invalidation on password change via `users.password_version`. Caveat: tutti gli utenti loggati al deploy si sloggano una volta al primo refresh (il JWT non aveva la nuova claim).
> - PR #44 (`70e5b9d`): **Sessione 6** — audit log applicativo (`public.audit_log` + helper `lib/db/audit-log.ts:logAudit` + 17 action sensibili tracciate: utenti, auth, movimenti, rate, bulk import).
> - PR #45 (`fab34e1`): **Sessione 7** — caching esteso + RPC `saldi_per_conto`. `unstable_cache` su `listMovimenti`/`listMovimentiInRange`/`saldiPerConto` con tag `movimenti` + `revalidateTag` sulle 6 mutation. `/cassa` + `/dashboard` usano la RPC per i KPI saldo. Bug fix collaterale sul dashboard che prima sommava i giroconti.
>
> **Branch produzione `claude/n8n-association-management-Q4pBM` al commit `fab34e1`.** Build verde, 31 rotte. Le tre sessioni SECURITY_PLAN (5+6+7) sono in finestra di osservazione produzione fino a:
> - Sessione 5 (auth): **2026-05-22** (1 settimana, tocca auth)
> - Sessione 6 (audit log): **2026-05-17** (48h, nuova tabella)
> - Sessione 7 (caching/RPC): **2026-05-17** (48h, nuova RPC + caching)
>
> **Sessioni SECURITY_PLAN in osservazione precedenti** (gia' chiuse nei giorni scorsi): Sessione 2 (PR #28, upload hardening), Sessione 3 (PR #29, defense-in-depth auth), Sessione 4 (PR #32 + #33, rate-limit login).
>
> **Smoke test consigliati post-deploy** (manuali da farsi in finestra incognito separata):
> 1. Login utente A in finestra 1 → login utente A in finestra 2 incognito → in 1 cambia password → in 2 refresh → deve essere sloggato (Sessione 5).
> 2. Admin resetta password utente B → B refresh → /login (Sessione 5).
> 3. Coordinatore appena creato: login con temp → /primo-accesso → cambia password → atterra /dashboard senza re-login (fix PR #38/#39 + Sessione 5).
> 4. SQL Editor Supabase: `select at, user_email, action, entity_type, entity_id, ip from public.audit_log order by at desc limit 20;` (Sessione 6).
> 5. SQL Editor: `select diff from public.audit_log where action like 'user.password%';` → verifica nessuna password/hash leakata (Sessione 6).
> 6. `/cassa` mostra gli stessi numeri di KPI di prima del merge (verifica numerica vs storico); `/dashboard` Saldo totale coerente con `/cassa` (Sessione 7).
> 7. Crea movimento da `/spese-edu` → `/cassa` mostra il saldo aggiornato senza ritardo (cache tag invalidata).
>
> **Sessione 8 (futura)**: vedi `SECURITY_PLAN.md` sezione "Sessione 8 — DB transactions per race conditions". Tocca codice in produzione su entita' live (presenze, iscrizioni, turni). Richiede 3 RPC `SECURITY DEFINER` e UNIQUE constraints sui tre flussi concorrenti. Da fare quando le sessioni 5+6+7 hanno passato la finestra di osservazione.
>
> **Sessione precedente** della giornata: `claude/secplan-06-audit-log` (PR #44 `70e5b9d`, audit log applicativo). Migration aggiunge `public.audit_log` + 3 indici + RLS. Helper `logAudit` fire-and-forget. 17 action sensibili tracciate. Refactor `requireAdmin`/`requireEduOrAdmin` per ritornare la session (era void).
>
> **Sessione precedente** della giornata: `claude/secplan-05-jwt-session-invalidation` (PR #43 `23e327b`, JWT maxAge 14gg→24h + session invalidation). Migration aggiunge `users.password_version`; `getPasswordVersion`/`incrementPasswordVersion` helpers; `auth.config.ts:jwt` check token-vs-DB ad ogni request (eccetto signIn/update); le tre server action di cambio password chiamano `incrementPasswordVersion` e le due self-service usano `unstable_update({user: {passwordVersion: next}})` per allineare il JWT corrente. **Caveat**: utenti gia' loggati al deploy si sloggano una volta al primo refresh.
>
> **Bonus della giornata**: CSP flip a enforcing (PR #42 `fbeaa5f`), logo /login + favicon col logo ACLI (PR #41 `6375505`), fix accesso utente loop primo-accesso non-admin (PR #38 + #39 `06cd336`/`d75542f`).
>
> **Da fare lato utente** (non bloccante, qualsiasi momento): rimuovere `AIRTABLE_API_KEY` + `AIRTABLE_BASE_ID` dalle env Vercel Production + Preview (codice non le legge piu').
>
...
- **Supabase project** `aikforfebngrqfzdkowo` (`eu-central-1`, Postgres + RLS abilitata): backing store di tutto il dominio dell'app. Schema descritto in "Modello dati corrente" sotto.
...
- **Try/catch anti-crash** su login: se Supabase cade o il service-role key viene ruotato, l'utente vede "Email o password non corretti" invece di un 500.
...
- **Spese-edu** (`/spese-edu`): form unico (Tipo Entrata/Uscita, Importo, Data, Conto radio Cassa/BCC/Sumup, Categoria filtrata per tipo, Descrizione obbligatoria, Note). Submit → dialog conferma con riepilogo segnato/colorato → `creaMovimentoAction` salva su `public.movimenti` con `id` testuale `app_xxx` per non collidere con quelli del bot Telegram. Toast successo + reset form. Sidebar a destra mostra gli ultimi 5 movimenti dell'utente corrente.
...
- **Sync Movimenti** Google Sheet → Supabase (workflow n8n id `cmMaMjtv6xEzdZQC`): **event-driven** via `Google Sheets Trigger` con `event: rowAdded`, polling ogni minuto, fa partire il workflow solo quando viene aggiunta una riga al Sheet. HTTP Request POST a `/rest/v1/rpc/upsert_movimento_from_sheet` (Supabase RPC `SECURITY DEFINER` + `GRANT EXECUTE TO anon`): risolve `categoria_nome` -> `categoria_id` via lookup case-insensitive, fa `INSERT ... ON CONFLICT ("timestamp") DO UPDATE` aggiornando solo i campi mutabili. Il nodo `Normalize for Supabase` forza `telegram_user_id` e `id_correzione` a stringa via `String(...)` perché il Sheet API restituisce gli ID Telegram come number. La pagina `/cassa` legge da `public.movimenti` (filtro `stato != 'errato'`).
...
|---|---|---|---|
| `claude/pricing-quota-sconti-iscrizioni` | 2026-05-16 | 👀 in review | **Pricing rifondato + quota + sconti + landing iscrizioni**. Migration Supabase: `modalita_iscrizione.tipo_prezzo` (`per_sessione`/`flat`), `attivita.quota_iscrizione`, `rate.tipo_riga`+`descrizione_riga`, nuove tabelle `sconti_attivita`+`iscrizioni_sconti`. Calcolo rate: 1 sola rata `pacchetto` se flat (era N rate × importo, bug); 1 rata `quota_iscrizione` opzionale via checkbox; 1 riga negativa `sconto` per ogni regola applicata (% imponibile=modalita+quota, oppure € fisso). `updateIscrizioneAction` con wipe-and-recreate se cambia policy/modalita (bloccato da `hasAnyRataPagata`), diff incrementale altrimenti. UI: `modalita-editor` con radio tipoPrezzo, `attivita-form` con quotaIscrizione, nuovo `sconti-editor` con DeleteConfirmDialog impact-aware (FK ON DELETE RESTRICT). `iscrizione-form` con sezioni quota/sconti + box anteprima totale (subtotale + quota + sconti = totale). `/iscrizioni` ridisegnata come card grid per attivita con drill-down `/iscrizioni/attivita/[id]` (KPI iscritti/incassato/residuo + tabs stato). |
| `claude/secplan-07-performance` | 2026-05-15 | 🌐 in osservazione | Sessione 7 SECURITY_PLAN: caching esteso + RPC aggregati — PR #45 mergeata (`fab34e1`). RPC `saldi_per_conto` + `unstable_cache` su `listMovimenti`/`listMovimentiInRange`/`saldiPerConto` + `revalidateTag("movimenti","max")` sulle mutation. KPI cassa/dashboard via RPC. Finestra 48h fino al 2026-05-17. |
| `claude/secplan-06-audit-log` | 2026-05-15 | 🌐 in osservazione | Sessione 6 SECURITY_PLAN: audit log applicativo — PR #44 mergeata (`70e5b9d`). 17 action sensibili tracciate. Finestra 48h fino al 2026-05-17. |
| `claude/secplan-05-jwt-session-invalidation` | 2026-05-15 | 🌐 in osservazione | Sessione 5 SECURITY_PLAN: JWT maxAge 14gg→24h + session invalidation on password change via `users.password_version` — PR #43 mergeata (`23e327b`). Finestra fino al 2026-05-22. Caveat: tutti gli utenti loggati al deploy si sloggano una volta al primo refresh. |
| `claude/review-security-progress-LLf7T` | 2026-05-14 | 🌐 in osservazione | Sessione 4 SECURITY_PLAN: rate limiting `/login` con Upstash sliding window (5 tent / 15 min per coppia IP+email) — PR #32 mergeata (`c7ffd5d`) + hotfix logging PR #33 (`a217d06`). Finestra di osservazione fino al 2026-05-21. |
| `claude/secplan-03-auth-defense-in-depth` | 2026-05-14 | 🌐 in osservazione | Sessione 3 SECURITY_PLAN: defense-in-depth auth + error message hardening — PR #29 mergeata (`8cf7ee3`). |
| `claude/secplan-02-upload-hardening` | 2026-05-14 | 🌐 in osservazione | Sessione 2 SECURITY_PLAN: file upload hardening + CSV formula injection — PR #28 mergeata (`ff4ba17`). |
| _Mergeato in produzione_ | — | ✅ | Ultime mergeate: PR #45 (`fab34e1`, caching + RPC saldi) + PR #44 (`70e5b9d`, audit log applicativo) + PR #43 (`23e327b`, JWT maxAge + session invalidation) + PR #42 (`fbeaa5f`, CSP enforcing flip) + PR #41 (`6375505`, logo /login + favicon col logo ACLI) + PR #39 (`d75542f`, primo-accesso usa unstable_update invece di signOut) + PR #38 (`06cd336`, rompe loop redirect /primo-accesso per non-admin). Precedenti: PR #35 (cleanup post-cutover Supabase, `0f41b01`), PR #34 (docs sync sessione 4), PR #33 (hotfix logging rate-limit), PR #32 (rate limiting /login con Upstash), PR #29 (defense-in-depth auth + error hardening), PR #28 (file upload hardening + CSV injection), PR #27 (security headers + CSP Report-Only), PR #24 (dettaglio voce ETS), PR #22 (lista inline movimenti da classificare), PR #21 (import estratto conto BCC/SumUp + rendiconto ETS), PR #20 (perf audit), PR #19 (cutover Supabase). |
...
### Cleanup post-cutover ✅ (fatto 2026-05-14)

Eseguito al termine della finestra di osservazione del cutover (PR #19 mergeata il 2026-05-11). Cosa e' cambiato:

- `lib/airtable/` cancellata. I 14 helper data-access (`bambini.ts`, `iscrizioni.ts`, ecc.) e il `client.ts` erano gia' inutilizzati: il sed globale di PR #19 aveva ripuntato tutto a `lib/db/`. Tipi di dominio (`User`, `Bambino`, `Movimento`, ecc.) trasferiti inline da `lib/airtable/types.ts` dentro `lib/db/types.ts` (era un re-export shim).
- `scripts/migrate-airtable-to-supabase.ts` cancellato (one-off del cutover, gia' eseguito su Supabase prod).
- `package.json`: `pnpm remove airtable` (rimossi 10 pacchetti dal lockfile) + cancellata la riga `"migrate:airtable"` dagli script.
- `.env.example`: rimosse `AIRTABLE_API_KEY` e `AIRTABLE_BASE_ID`.
- **Da fare manualmente lato utente su Vercel** (post-merge della PR di cleanup): rimuovere `AIRTABLE_API_KEY` e `AIRTABLE_BASE_ID` dalle env Production + Preview. Non bloccante: lasciate li' non rompono niente, ma sono dead weight.
- Build verde post-cleanup: `pnpm typecheck && pnpm lint && pnpm build` puliti, 31 rotte invariate.

### Rollback rapido del cutover (storico — sempre disponibile finche' Supabase regge)

In caso si scoprano regressioni gravi imputabili al cutover Supabase (cioe' su PR #19 e non sul cleanup):
- `git revert <merge-commit-#19>` sul branch produzione + push.
- Rimettere env `AIRTABLE_API_KEY` + `AIRTABLE_BASE_ID` su Vercel Production + Preview.
- Lo schema Supabase resta in piedi inutilizzato — non sono necessarie azioni distruttive.
- Nota post-cleanup: il revert dovra' anche ripristinare `lib/airtable/`, lo script di migrazione, e la dep `airtable`. Tutto recuperabile da git history.
...
| 4b | ~~Aggregati SQL invece di `.filter().reduce()` in JS~~ | 🟡 parzialmente risolto | Sessione 7 SECURITY_PLAN (PR #45) ha portato i KPI saldo di `/cassa` + `/dashboard` su RPC `saldi_per_conto`. Rimasti aggregati JS: dashboard "entrate/uscite del mese" e `/cassa` "totale periodo filtrato" (subset client-side della tabella). Da risolvere quando il volume mensile supera ~1000 movimenti: aggiungere RPC `totali_per_mese(YYYY-MM)` e migrare i filtri del footer cassa al DB. |
...
- **Ultimo branch mergeato**: `claude/secplan-07-performance` (PR #45, caching esteso + RPC `saldi_per_conto`). Branch precedenti: `claude/secplan-06-audit-log` (PR #44, audit log applicativo), `claude/secplan-05-jwt-session-invalidation` (PR #43, JWT maxAge 24h + session invalidation), `claude/csp-enforcing-flip` (PR #42, CSP flip enforcing), `claude/fix-logo-favicon-and-proxy-matcher` (PR #41), `claude/fix-login-access-fOBuD` (PR #38 + #39), `claude/cleanup-airtable-postcutover` (PR #35), `claude/docs-secplan-s4-postmerge` (PR #34), `claude/review-security-progress-LLf7T` (PR #32 + PR #33), `claude/secplan-03-auth-defense-in-depth` (PR #29), `claude/secplan-02-upload-hardening` (PR #28), `claude/security-plan-session-1-UNFfC` (PR #27).
...
- **Workflow n8n sync Movimenti**: `cmMaMjtv6xEzdZQC` ([link](https://eurita.app.n8n.cloud/workflow/cmMaMjtv6xEzdZQC)) — Google Sheet `Cassa_Associazione_Template` (id `1NZ9G7Vv8C6yYMb-oA3czSNq4d1vVC961iIMOXtAnrqE`) → Supabase `public.movimenti` (via RPC `upsert_movimento_from_sheet`). **Trigger**: `Google Sheets Trigger` su `event: rowAdded`, polling ogni minuto (parte solo su nuova riga). **Match**: `timestamp` (ms-precision dal bot Telegram, `ON CONFLICT (timestamp) DO UPDATE` lato Postgres).
- **Env vars necessarie su Vercel** (Production + Preview): `AUTH_SECRET`, `AUTH_TRUST_HOST=true`, `SUPABASE_URL=https://aikforfebngrqfzdkowo.supabase.co`, `SUPABASE_SERVICE_ROLE_KEY` (formato `sb_secret_*`), `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN` (rate limiting `/login`). **Dopo la PR di cleanup post-cutover**: rimuovere `AIRTABLE_API_KEY` + `AIRTABLE_BASE_ID` (non piu' usate dal codice).

## Stack

Next.js 16 (App Router, React 19, Turbopack) · Tailwind CSS v4 · Auth.js v5 · Supabase (Postgres) · Upstash Redis · n8n
