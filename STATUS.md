...
> Ultimo aggiornamento: **2026-05-13** (notte) — **import estratto conto BCC/SumUp + rendiconto ETS** sul branch `claude/add-bank-statement-sync-6jLq3`. 5 commit atomici, branch non ancora mergeato (vedi sezione "In review"). Risponde a due richieste utente: (1) caricare gli estratti conto della banca per intercettare le spese che non passano da Telegram (bollette domiciliate, commissioni, gite, ecc.) deduplicandole contro i movimenti gia registrati; (2) attribuire a ogni movimento una "voce di rendiconto" cosi che entrate/uscite si possano raggruppare nello schema obbligatorio del rendiconto per cassa ETS (D.M. 5/3/2020 — Modello D, 49 voci su 5 sezioni A-E per entrate e uscite). Cambio strutturale collaterale: ora **segnare una rata come pagata crea un Movimento Entrata** sul conto scelto (Cassa/BCC/Sumup), linkato via `rate.movimento_id`. Prima la rata pagata viveva solo nella tabella `rate` e il bank import non poteva matcharla. **Da fare in produzione dopo merge**: girare `pnpm backfill:rate-movimenti` per generare retroattivamente i Movimenti delle rate gia pagate.
>
...
|---|---|---|---|
| `claude/add-bank-statement-sync-6jLq3` | 2026-05-13 | da mergeare | Import estratto conto BCC/SumUp + rendiconto ETS + voci rendiconto su categorie/movimenti. Schema migration applicate su Supabase produzione (vedi sezione "Stato post-cutover" / "Fatto"). Dopo merge: girare `pnpm backfill:rate-movimenti` per generare i movimenti Entrata storici delle rate gia pagate. Tutti i check verdi (typecheck + lint + build). |
...
- **Ultimo branch mergeato**: `claude/review-site-architecture-TBDbq` (PR #20, perf audit). Branch precedente: `claude/airtable-to-supabase-migration-uBZxn` (PR #19, cutover Supabase).
