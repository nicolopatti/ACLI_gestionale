...
>
> Ultimo aggiornamento: **2026-05-16** — **Pricing iscrizioni rifondato**: modalita ora hanno `tipo_prezzo: per_sessione | flat` (default per_sessione = retrocompat), quota iscrizione una-tantum opzionale per attivita, sconti configurabili (% o €) applicabili come checkbox al momento dell'iscrizione, pagina `/iscrizioni` rifatta come landing card-per-attivita con drill-down `/iscrizioni/attivita/[id]`. Branch in attesa di merge: `claude/pricing-quota-sconti-iscrizioni`. **SECURITY_PLAN**: in attesa la sola sessione 8 (DB transactions, rischio 🔴 alto), da affrontare dopo finestra di osservazione delle PR #43/#44/#45 (riapertura dal 2026-05-17).
>
