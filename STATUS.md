...
>
> Ultimo aggiornamento: **2026-05-14** — **Sessioni 1+2+3 SECURITY_PLAN tutte mergeate in produzione**, in finestra di osservazione. Branch produzione `claude/n8n-association-management-Q4pBM` al commit `8cf7ee3`. **PR #27** (sessione 1 — HTTP security headers + CSP Report-Only): 5 header standard + CSP Report-Only in `next.config.ts`, dopo 24h senza violazioni si flippa la key a `Content-Security-Policy` per attivare l'enforcing. **PR #28** (sessione 2 — file upload hardening + CSV formula injection): `validateUploadedFile` client (`MAX_FILE_SIZE=5MB` + whitelist `[.csv,.tsv,.xls,.txt]`) + `MAX_FILE_TEXT_SIZE=6MB` server in `parseEstrattoContoAction` (chiude DoS via `curl`) + `sanitizeFormula`+`escText` in `rendicontoToCsv` (prepend apostrofo se cella inizia per `=+-@\t\r`, applicato ai campi testo, numeri restano nativi per le somme) + cap descrizione importata a 1000 char (`lib/import/types.ts` + parsers BCC/SumUp). **PR #29** (sessione 3 — defense-in-depth auth + error hardening): `lib/errors.ts` (`BusinessError` + `userErrorMessage`), `lib/auth/page-guards.ts` (`requireAdmin` + `requireAdminOrCoordinatore`), `auth()` esplicito su 16 pagine (2 admin-only `utenti/*` + 14 area educativa), error handler riscritti su 13 file di action (Postgres errors tipo `duplicate key value violates unique constraint` non leakano piu' al client), sweep di 33 import `@/lib/airtable/types` → `@/lib/db/types` via re-export shim. Build verde post-merge, 29 rotte invariate. **Prossimo step a tua scelta**: (a) flip CSP a enforcing in `next.config.ts` (rinomina la key da `Content-Security-Policy-Report-Only` a `Content-Security-Policy`); (b) sessione 4 (rate limiting su `/login` con Upstash, richiede env `UPSTASH_REDIS_REST_URL` + `UPSTASH_REDIS_REST_TOKEN` su Vercel).
>
...
| `claude/security-plan-session-1-UNFfC` | 2026-05-14 | 🌐 in osservazione | Sessione 1 SECURITY_PLAN: HTTP security headers + CSP Report-Only — PR #27 mergeata (`3342fe9`). Finestra 24h prima del flip key `Content-Security-Policy-Report-Only` → `Content-Security-Policy`. |
| _Mergeato in produzione_ | — | ✅ | Ultima mergeata: PR #29 (defense-in-depth auth + error hardening). Precedenti: PR #28 (file upload hardening + CSV injection), PR #27 (security headers + CSP Report-Only), PR #24 (dettaglio voce ETS), PR #22 (lista inline movimenti da classificare), PR #21 (import estratto conto BCC/SumUp + rendiconto ETS), PR #20 (perf audit), PR #19 (cutover Supabase). |
