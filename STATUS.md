...
>
> Ultimo aggiornamento: **2026-05-15** — **Separato "saldi reali conti" da "rendiconto finanziario"**: nuova rotta `/conti` (admin + volontario_cassa) con i saldi REALI per Cassa/BCC/SumUp **inclusi i giroconti** (un versamento Cassa→BCC abbassa Cassa e alza BCC come deve essere) + lista delle 5 ultime operazioni per conto. La rotta `/cassa` diventa la vista rendiconto finanziario: 3 KPI (Entrate anno, Uscite anno, Risultato netto storico) + chart mensile entrate/uscite via `recharts` + lista movimenti **esclusi giroconti**. Nuove RPC Supabase: `saldi_per_conto_reale` (include giroconti) e `entrate_uscite_per_mese`. `saldiPerConto` esistente invariato (esclude giroconti, semantica rendiconto). Sidebar amm: aggiunta voce "Saldi conti" (icona `Wallet`). Caso reale: BCC oggi sembrava −5560,71€ (escluso giroconti), saldo reale −2445,71€ (con i ~3115€ di versamenti contanti incoming flaggati `is_giroconto=true`). Branch: `claude/fix-sumup-statement-import-5coBD` (continua dalla sessione import SumUp+BCC). Aggiornamento precedente del 2026-05-16: pricing iscrizioni rifondato (modalita `tipo_prezzo`, quota una-tantum, sconti). **SECURITY_PLAN**: in attesa la sola sessione 8 (DB transactions, rischio 🔴 alto).
>
